...
impl Plugin for LogPlugin {
    fn build(&self, app: &mut App) {
        ...
        // PATCHED: emscripten (Godot web) joins the native path — it has a real
        // std (stderr → browser console) but no working tracing_wasm/wasm-bindgen.
        #[cfg(all(any(not(target_arch = "wasm32"), target_os = "emscripten"), not(target_os = "ios")))]
        {
        ...
        // PATCHED: real browser wasm only; emscripten uses the native path above.
