...
/// https://github.com/avianphysics/avian/blob/main/crates/avian2d/examples/dynamic_character_2d/plugin.rs#L6
...
impl Default for MovementBundle {
    fn default() -> Self {
        let acc = 300.;
        Self::new(acc, 2.5, 80.0, PI * 0.45, acc * 1.5)
    }
}
...
impl CharacterControllerBundle {
    pub fn new(collider: Collider) -> Self {
        ...
        Self {
            ...
            collider,
            ground_caster: ShapeCaster::new(caster_shape, Vec2::ZERO, 0.0, Dir2::NEG_Y)
...
/// Responds to [`MovementAction`] events and moves character controllers
/// accordingly.
fn movement(
    time: Res<Time>,
        ...
        ) in &mut controllers
        {
            match event {
                MovementAction::Move(direction) => {
                    let mut actual_acc = movement_acceleration.0;
                    if movement_acceleration.0 >= max_acceleration.0 {
                        actual_acc = max_acceleration.0
                    }
                    linear_velocity.x += *direction * actual_acc * delta_time;
                },
                MovementAction::Jump => {
                    if is_grounded {
...
fn apply_movement_damping(
    ...
    for (damping_factor, mut linear_velocity) in &mut query {
        // We could use `LinearDamping`, but we don't want to dampen movement along the
        // Y axis
