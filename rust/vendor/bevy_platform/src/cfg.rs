...
define_alias! {
    ...
    // PATCHED: exclude emscripten. Godot web export targets
    // wasm32-unknown-emscripten, which has a real libc/std but NO functional
    // wasm-bindgen (js-sys panics at runtime). Excluding emscripten here makes
    // `cfg::web` false, so bevy falls through to its native/std branches
    // (e.g. bevy_tasks uses the std LOCAL_EXECUTOR instead of web-task).
    #[cfg(all(target_arch = "wasm32", not(target_os = "emscripten"), feature = "web"))] => {
